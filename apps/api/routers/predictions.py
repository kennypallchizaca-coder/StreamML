...
async def _blocked(
    ...
    record = request.app.state.database.store_prediction(
        user_id=user["id"], session_id=session_id, model_role=role,
...
async def _executed(
    request: Request, user: dict, session_id: str, role: str, result: dict, fingerprint: str
) -> dict:
    record = request.app.state.database.store_prediction(
        user_id=user["id"], session_id=session_id, model_role=role,
        model_version=request.app.state.registry.version, status="executed", result=result,
...
async def predict_reactive(
    payload: ReactivePredictionRequest, request: Request, user: dict = Depends(current_user)
):
...
async def predict_predictive(
    ...
    fingerprint = request.app.state.engine.fingerprint(
        {"samples": samples, "current_profile": payload.current_profile}
    )
    try:
