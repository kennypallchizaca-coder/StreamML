...
def _connector_instance_is_running() -> bool:
    """Check the shared lock so the GUI reflects a monitor it did not spawn."""

    probe = ConnectorInstanceLock()
    try:
        probe.acquire()
...
class SetupService:
    ...
    def state(self) -> dict[str, Any]:
        ...
        connector_running = process_running or _connector_instance_is_running()
        return {
            "connector": connector,
    ...
    def start_connector(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            if _connector_instance_is_running():
                return {
                    "message": "Ya hay un conector StreamML activo para este OBS.",
                    "pid": None,
                    "state": self.state(),
                }
...
function renderProductionReadiness(next){const d=next.deployment||{},s=next.secrets||{},c=next.capabilities||{},checks=[{label:'Docker Compose',ready:Boolean(c.docker_available&&c.compose_file_available),detail:'Docker y la infraestructura están disponibles.'},{label:'Origen HTTPS',ready:String(d.public_origin||'').startsWith('https://'),detail:'Dominio público configurado.'},{label:'Certificados TLS',ready:Boolean(d.tls_cert_file&&d.tls_key_file),detail:'Rutas de certificado y clave configuradas.'},{label:'Secretos del servidor',ready:Boolean(s.deployment_token_secret&&s.deployment_media_auth_secret&&s.deployment_bootstrap_password),detail:'Credenciales almacenadas de forma cifrada.'},{label:'WebRTC',ready:Boolean(d.mediamtx_webrtc_additional_hosts),detail:'Host público de MediaMTX configurado.'}];const root=document.querySelector('#server-readiness-items');root.replaceChildren(...checks.map(item=>{const el=document.createElement('div');el.className='readiness-item '+(item.ready?'ok':'pending');el.textContent=item.label+': '+(item.ready?item.detail:'Pendiente');return el}));const ready=checks.every(item=>item.ready),title=document.querySelector('#server-readiness-title'),detail=document.querySelector('#server-readiness-detail');title.textContent=ready?'Listo para validar el despliegue':'Faltan requisitos de producción';detail.textContent=ready?'Pulsa “Validar preparación” antes de iniciar o actualizar los servicios.':'Completa los elementos marcados como pendientes. La validación comprobará Docker Compose y TLS.'}
function apply(next){state=next;fill('#connector-form',state.connector);fill('#server-form',state.deployment);const s=state.secrets||{},c=state.capabilities||{},running=Boolean(c.connector_running),pill=document.querySelector('#runtime-pill');secret('#obs-secret',s.obs_websocket_password);secret('#bootstrap-secret',s.deployment_bootstrap_password);secret('#restream-secret',s.deployment_restream_config_json);secret('#token-secret',s.deployment_token_secret);secret('#media-secret',s.deployment_media_auth_secret);pill.className='runtime '+(running?'running':'offline');document.querySelector('#runtime-text').textContent=running?'Monitoreo activo':'Monitoreo detenido';document.querySelector('#connector-runtime').textContent=running?(c.connector_pid?'Activo · PID '+c.connector_pid:'Activo · instancia protegida'):'Detenido';document.querySelector('#docker-runtime').textContent=c.docker_available?'Disponible':'No detectado';renderProductionReadiness(state)}
