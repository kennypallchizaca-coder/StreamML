...
def main() -> None:
    ...
    predictive_row = predictive_data.loc[
        predictive_data["session_id"].isin(predictive_manifest["splits"]["test"])
    ].iloc[[0]]
    ...
    print(
        json.dumps(
            {
                "reactive": {
                    "prediction": str(reactive_model.predict(reactive_x)[0]),
                    "actual": reactive_row["target"].iloc[0],
                },
                "predictive": {
