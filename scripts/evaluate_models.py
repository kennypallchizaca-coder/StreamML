...
from src.streamml.training.predictive import evaluate_saved_release
from src.streamml.services.release import file_hashes, read_json, requirements_snapshot, write_json
...
def main() -> None:
    config = read_json(ROOT / "src" / "streamml" / "config" / "dataset_config.json")
    predictive = evaluate_saved_release(ROOT, config)
    reactive = read_json(ROOT / "models" / "registry" / "reactive" / "metrics.json")
    reactive_training = read_json(ROOT / "models" / "registry" / "reactive" / "training_manifest.json")
