...
from src.reactive_dataset import REACTIVE_CLASSES, REACTIVE_FEATURES, PROFILE_TO_CODE
from src.release_utils import classification_metrics, requirements_snapshot, sha256_file, write_json


def load_contract(root: Path) -> dict[str, Any]:
    return json.loads((root / "config" / "reactive_feature_contract.json").read_text(encoding="utf-8"))
...
def train_reactive_release(root: Path) -> dict[str, Any]:
    ...
    release_dir = root / "models" / "release" / "reactive"
    release_dir.mkdir(parents=True, exist_ok=True)
    joblib.dump(final_model, release_dir / "model.joblib")
    shutil.copyfile(root / "config" / "reactive_feature_contract.json", release_dir / "feature_contract.json")
    ...
    manifest = {
        ...
        "model_role": "reactive",
        "model_path": "models/release/reactive/model.joblib",
