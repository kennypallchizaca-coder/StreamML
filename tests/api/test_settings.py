...
def test_settings_are_persisted_and_password_change_requires_current_password(client: TestClient):
    ...
    preferences = {
        "language": "es", "timezone": "America/Guayaquil", "dark_mode": False,
        "alert_detail": "high",
    }
    assert client.put("/api/v1/settings/preferences", json=preferences).status_code == 200
    stream = {
        "preferred_resolution": "720p", "preferred_profile": "medium", "platform": "twitch",
        "live_scene": "Escena principal", "backup_scene": "Escena respaldo",
        "network_probe_interval_seconds": 9, "network_probe_bytes": 65536,
    }
    assert client.put("/api/v1/settings/stream", json=stream).status_code == 200
    assert client.put("/api/v1/settings/account", json={
        "display_name": "Alexis", "current_password": "incorrect-password", "new_password": "New-Secure-Password-2026",
    }).status_code == 400
    updated = client.put("/api/v1/settings/account", json={
        "display_name": "Alexis", "current_password": TEST_PASSWORD,
    ...
    assert client.post("/api/v1/auth/login", json={
        "email": "owner@example.com", "password": "New-Secure-Password-2026",
    }).status_code == 200


def test_new_session_uses_configured_defaults_and_external_video_link_is_persisted(client: TestClient):
    login(client)
    assert client.put("/api/v1/settings/stream", json={
        "preferred_resolution": "720p", "preferred_profile": "medium", "platform": "kick",
    ...
    created = client.post("/api/v1/sessions", json={
        "name": "Evento IRL", "planned_duration_hours": "4", "connection_type": "mobile",
    })
    assert created.status_code == 201
    session = created.json()
    assert session["configuration"] == {
        "platform": "kick", "resolution": "720p", "initial_profile": "medium",
        "planned_duration_hours": "4", "connection_type": "mobile",
    }
...
def test_connector_receives_saved_runtime_settings_and_history_delete_preserves_preferences(client: TestClient):
    ...
    stream = {
        "preferred_resolution": "480p", "preferred_profile": "low", "platform": "youtube",
        "live_scene": "Live personalizado", "backup_scene": "Respaldo personalizado",
        "network_probe_interval_seconds": 11, "network_probe_bytes": 32768,
    }
    assert client.put("/api/v1/settings/stream", json=stream).status_code == 200
    code = client.post("/api/v1/pairing/codes", json={"session_id": session["id"]}).json()["code"]
    linked = client.post("/api/v1/connectors/link", json={
        "code": code, "connector_name": "OBS local", "connector_version": "1.0.0",
    })
