...
from src.predictive_training import positive_probability


ROOT = Path(__file__).resolve().parents[1]
PREDICTIVE_RELEASE = ROOT / "models" / "release" / "predictive"
...
def test_required_release_statuses_are_offline() -> None:
    manifest = load_manifest()
    ...
    assert manifest["offline_release"] is True


def test_demo_is_offline_and_completes() -> None:
