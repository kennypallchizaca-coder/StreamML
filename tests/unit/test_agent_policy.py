...
def test_upgrade_requires_hysteresis_and_respects_one_level() -> None:
    agent = AutonomousStreamingAgent(AgentPolicy(upgrade_confirmations=3, minimum_change_interval_seconds=0))
...
def test_upgrade_cooldown_resets_confirmation_streak() -> None:
    agent = AutonomousStreamingAgent(AgentPolicy(upgrade_confirmations=2, minimum_change_interval_seconds=30))
...
def test_signal_loss_backup_and_stable_recovery() -> None:
    agent = AutonomousStreamingAgent(AgentPolicy(signal_loss_grace_seconds=3, recovery_stable_seconds=10))
    state = AgentState()
...
def test_state_roundtrip() -> None:
    state = AgentState(
        ...
        backup_active=True,
        last_profile_change_at=1.5,
        signal_lost_at=2.0,
        signal_recovered_at=3.0,
        upgrade_streak=4,
    )
